...
public sealed class DecisionSuggestionService
{
    ...
    private static DecisionSuggestion BuildSuggestion(RawCandidate candidate)
    {
        ...
        decimal actionScore = CalculateActionScore(candidate);
        ...
        decimal decisionScore =
            Clamp(
                ...
                crowdScore * 0.10m +
                (100m - riskScore) * 0.05m,
                0m,
                100m);

        if (candidate.RecentMovePercent is null)
        {
            decisionScore = Math.Min(decisionScore, 45m);
        }
        ...
        decisionScore = ApplyLabelScoreCap(label, decisionScore);
        ...
        decimal actionScore)
    {
        if (candidate.RecentMovePercent is null)
        {
            if (liquidityScore >= 70m && riskScore <= 30m)
                return "BASELINE_RESEARCH_CANDIDATE";
        ...
        if (actionScore < 25m && veryLiquid && riskScore <= 25m)
        {
            return "BASELINE_RESEARCH_CANDIDATE";
        }
        ...
        if (candidate.RecentMovePercent is >= 0.15m &&
            actionScore >= 40m &&
            decisionScore >= 48m &&
            riskScore < 55m)
        {
            return "EARLY_ACTION_WATCH";
        }

        if (decisionScore >= 35m && riskScore < 60m)
            return "WATCH";

        return "AVOID";
    }

    private static decimal ApplyLabelScoreCap(string label, decimal score)
    {
        return label switch
        {
            "EARLY_ACTION_WATCH" => Math.Min(score, 69m),
            "BASELINE_RESEARCH_CANDIDATE" => Math.Min(score, 55m),
            "COLLECT_MORE_DATA" => Math.Min(score, 40m),
            "WATCH" => Math.Min(score, 54m),
            "AVOID" => Math.Min(score, 30m),
            _ => score
        };
    }

    private static decimal CalculateActionScore(RawCandidate candidate)
    {
        decimal recentAbs = Math.Abs(candidate.RecentMovePercent ?? 0m);
        decimal recentUp = candidate.RecentMovePercent ?? 0m;
        decimal day = candidate.PriceChange24hPercent ?? 0m;

        decimal recentMoveScore = recentAbs switch
        {
            >= 2.0m => 55m,
            >= 1.0m => 42m,
            >= 0.5m => 30m,
        ...
        decimal daySupportScore = day switch
        {
            >= 8m => 15m,
            >= 3m => 10m,
            >= 0m => 5m,
        ...
        decimal activityScore = candidate.QuoteVolume24h switch
        {
            >= 100_000_000m => 15m,
            >= 25_000_000m => 10m,
            >= 5_000_000m => 5m,
        ...
        return Clamp(
            recentMoveScore + directionScore + daySupportScore + activityScore,
            0m,
            100m);
    }
    ...
    private static (int? MinMinutes, int? MaxMinutes) GetHorizon(string label)
    {
        return label switch
        {
            ...
            "EARLY_ACTION_WATCH" => (5, 30),
            "BASELINE_RESEARCH_CANDIDATE" => (null, null),
            "COLLECT_MORE_DATA" => (null, null),
            "WATCH" => (null, null),
            "AVOID" => (null, null),
            _ => (null, null)
        };
    ...
    private static string BuildReason(RawCandidate candidate, string label)
    {
        ...
        return label switch
        {
            ...
            "EARLY_ACTION_WATCH" =>
                $"{recentText}. {volumeText}, and {spreadText}. In simple words: the coin is starting to move, but the setup is not clean enough yet to call it a short-term scalp candidate.",

            "WATCH" =>
                $"{recentText}. {volumeText}, and {spreadText}. In simple words: this is worth watching, but there is no strong action signal yet.",

            "BASELINE_RESEARCH_CANDIDATE" =>
                $"{volumeText}, and {spreadText}. In simple words: this pair is liquid and clean enough to keep on the radar, but there is not enough recent enriched history yet to call it a short-term setup.",

            "COLLECT_MORE_DATA" =>
                $"{volumeText}, and {spreadText}. In simple words: the app needs more enriched snapshots before it can judge this pair properly.",
        ...
        decimal riskScore)
    {
        ...
        string prefix = label switch
        {
            ...
            "EARLY_ACTION_WATCH" => "This is early and unconfirmed.",
            "WATCH" => "This is not ready yet.",
            "BASELINE_RESEARCH_CANDIDATE" => "This is a radar candidate, not a trade signal yet.",
            "COLLECT_MORE_DATA" => "The main risk is that the app has too little enriched history to judge recent behavior.",
    ...
    private static string BuildInvalidationNote(string label)
    {
        return label switch
        {
            ...
            "EARLY_ACTION_WATCH" =>
                "Invalid if the next scrape does not continue the move, volume fades, or the spread widens.",

            "WATCH" =>
                "Needs more evidence: better volume, cleaner spread, or stronger movement.",

            "BASELINE_RESEARCH_CANDIDATE" =>
                "Needs at least two enriched scrapes before this can become a short-term setup.",

            "COLLECT_MORE_DATA" =>
                "Run the collector again and wait for more comparable snapshots.",
